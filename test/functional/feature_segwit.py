...
from test_framework.script import CScript, OP_HASH160, OP_CHECKSIG, OP_0, hash160, OP_DUP, OP_EQUALVERIFY, OP_1, OP_2, OP_CHECKMULTISIG, OP_TRUE, OP_DROP, GetP2SH
...
class SegWitTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        solvable_after_importaddress.append(GetP2SH(script))
    ...
    def p2sh_address_to_script(self, v):
        ...
        p2sh_p2wsh = GetP2SH(p2wsh)
        return([bare, p2sh, p2wsh, p2sh_p2wsh])

    def p2pkh_address_to_script(self, v):
        pubkey = hex_str_to_bytes(v['pubkey'])
        p2wpkh = CScript([OP_0, hash160(pubkey)])
        p2sh_p2wpkh = GetP2SH(p2wpkh)
        p2pk = CScript([pubkey, OP_CHECKSIG])
        p2pkh = CScript(hex_str_to_bytes(v['scriptPubKey']))
        p2sh_p2pk = GetP2SH(p2pk)
        p2sh_p2pkh = GetP2SH(p2pkh)
        p2wsh_p2pk = CScript([OP_0, sha256(p2pk)])
        p2wsh_p2pkh = CScript([OP_0, sha256(p2pkh)])
        p2sh_p2wsh_p2pk = GetP2SH(p2wsh_p2pk)
        p2sh_p2wsh_p2pkh = GetP2SH(p2wsh_p2pkh)
