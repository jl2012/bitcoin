...
from .messages import CTransaction, CTxOut, sha256, hash256, uint256_from_str, ser_uint256, ser_string, ser_compact_size
...
def TaprootSignatureHash(txTo, amounts, hash_type, spk, input_index = 0, scriptpath = False, tapscript = CScript(), codeseparator_pos = -1, annex = b''):
    ...
    if (spend_type & 2):
        ss += ser_uint256(uint256_from_str(SizedHash(annex)))
    ...
    if (scriptpath):
        ss += ser_uint256(uint256_from_str(SizedHash(tapscript)))
...
class Tapscript():
    ...
    def gethash(self, pubkey):
        return TaggedHash("TapLeaf", GetVersionTaggedPubKey(pubkey, self.version) + ser_compact_size(len(self.script)) + self.script)
...
class Taproot():
    ...
    def get_bech32(self, main = False):
        from .address import program_to_witness
        return program_to_witness(1, GetVersionTaggedPubKey(self.pubkey_output, TAPROOT_VER), main)

    def get_p2sh(self, main = False):
        from .address import scripthash_to_p2sh
        return scripthash_to_p2sh(hash160(self.get_spk()), main)
