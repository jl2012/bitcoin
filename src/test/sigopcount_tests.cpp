...
BOOST_AUTO_TEST_CASE(GetSigOpCount)
{
    ...
    CScript s1;
    BOOST_CHECK_EQUAL(s1.GetSigOpCount(SIGOPCOUNT_LEGACY), 0U);
    BOOST_CHECK_EQUAL(s1.GetSigOpCount(SIGOPCOUNT_P2SH), 0U);

    uint160 dummy;
    s1 << OP_1 << ToByteVector(dummy) << ToByteVector(dummy) << OP_2 << OP_CHECKMULTISIG;
    BOOST_CHECK_EQUAL(s1.GetSigOpCount(SIGOPCOUNT_P2SH), 2U);
    s1 << OP_IF << OP_CHECKSIG << OP_ENDIF;
    BOOST_CHECK_EQUAL(s1.GetSigOpCount(SIGOPCOUNT_P2SH), 3U);
    BOOST_CHECK_EQUAL(s1.GetSigOpCount(SIGOPCOUNT_LEGACY), 21U);
    ...
    BOOST_CHECK_EQUAL(s2.GetSigOpCount(SIGOPCOUNT_P2SH), 3U);
    BOOST_CHECK_EQUAL(s2.GetSigOpCount(SIGOPCOUNT_LEGACY), 20U);

    p2sh = GetScriptForDestination(CScriptID(s2));
    BOOST_CHECK_EQUAL(p2sh.GetSigOpCount(SIGOPCOUNT_P2SH), 0U);
    BOOST_CHECK_EQUAL(p2sh.GetSigOpCount(SIGOPCOUNT_LEGACY), 0U);
    CScript scriptSig2;
 ...
 */
ScriptError VerifyWithFlag(const CTransaction& output, const CMutableTransaction& input, int flags, const CAmount nFees = 0)
{
    ScriptError error;
    CTransaction inputi(input);
    bool ret = VerifyScript(inputi.vin[0].scriptSig, output.vout[0].scriptPubKey, &inputi.vin[0].scriptWitness, flags, TransactionSignatureChecker(&inputi, 0, output.vout[0].nValue, nFees), &error);
