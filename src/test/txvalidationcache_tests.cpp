...
BOOST_FIXTURE_TEST_CASE(tx_mempool_block_doublespend, TestChain100Setup)
{
    ...
    for (int i = 0; i < 2; i++)
    {
        ...
        uint256 hash = SignatureHash(scriptPubKey, spends[i], 0, SIGHASH_ALL, CTxOut(), SigVersion::BASE);
...
BOOST_FIXTURE_TEST_CASE(checkinputs_test, TestChain100Setup)
{
    ...
    {
        std::vector<unsigned char> vchSig;
        uint256 hash = SignatureHash(p2pk_scriptPubKey, spend_tx, 0, SIGHASH_ALL, CTxOut(), SigVersion::BASE);
        BOOST_CHECK(coinbaseKey.Sign(hash, vchSig));
    ...
    // Test CHECKLOCKTIMEVERIFY
    {
        ...
        // Sign
        std::vector<unsigned char> vchSig;
        uint256 hash = SignatureHash(spend_tx.vout[2].scriptPubKey, invalid_with_cltv_tx, 0, SIGHASH_ALL, CTxOut(), SigVersion::BASE);
        BOOST_CHECK(coinbaseKey.Sign(hash, vchSig));
        vchSig.push_back((unsigned char)SIGHASH_ALL);
    ...
    // TEST CHECKSEQUENCEVERIFY
    {
        ...
        // Sign
        std::vector<unsigned char> vchSig;
        uint256 hash = SignatureHash(spend_tx.vout[3].scriptPubKey, invalid_with_csv_tx, 0, SIGHASH_ALL, CTxOut(), SigVersion::BASE);
        BOOST_CHECK(coinbaseKey.Sign(hash, vchSig));
        vchSig.push_back((unsigned char)SIGHASH_ALL);
    ...
    // for the same tx with a different witness.
    {
        ...
        ProduceSignature(keystore, MutableTransactionSignatureCreator(&valid_with_witness_tx, 0, CTxOut(11*CENT, spend_tx.vout[1].scriptPubKey), SIGHASH_ALL), spend_tx.vout[1].scriptPubKey, sigdata);
...

    {
        ...
        for (int i=0; i<2; ++i) {
            SignatureData sigdata;
            ProduceSignature(keystore, MutableTransactionSignatureCreator(&tx, i, CTxOut(11*CENT, spend_tx.vout[i].scriptPubKey), SIGHASH_ALL), spend_tx.vout[i].scriptPubKey, sigdata);
