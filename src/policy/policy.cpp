...
bool IsWitnessStandard(const CTransaction& tx, const CCoinsViewCache& mapInputs)
{
    ...
    for (unsigned int i = 0; i < tx.vin.size(); i++)
    {
        ...
        if (p2wsh || taproot) {
            if (stack.back().size() > MAX_STANDARD_WITNESS_SCRIPT_SIZE)
                return false;
            stack.pop_back();
            if (stack.size() > MAX_STANDARD_WITNESS_INPUT_STACK_ITEMS)
                return false;
            for (unsigned int j = 0; j < stack.size(); j++) {
                if (stack[j].size() > MAX_STANDARD_WITNESS_INPUT_STACK_ITEM_SIZE)
                    return false;
            }
        }
    }
