                              ...
                              std::vector<uint256>& vHashTxnToUncache)
{
    ...
    bool mastEnabled = witnessEnabled && (VersionBitsTipState(chainparams.GetConsensus(), Consensus::DEPLOYMENT_MAST) == THRESHOLD_ACTIVE);
    ...
    string reason;
    if (fRequireStandard && !IsStandardTx(tx, reason, witnessEnabled, mastEnabled))
...

    {
        ...
        if (!CheckInputs(tx, state, view, true, scriptVerifyFlags, true, txdata, nFees)) {
            // SCRIPT_VERIFY_CLEANSTACK requires SCRIPT_VERIFY_WITNESS, so we
            // need to turn both off, and compare against just turning off CLEANSTACK
            // to see if the failure is specifically due to witness validation.
            if (tx.wit.IsNull() && CheckInputs(tx, state, view, true, scriptVerifyFlags & ~(SCRIPT_VERIFY_WITNESS | SCRIPT_VERIFY_CLEANSTACK), true, txdata, nFees) &&
                !CheckInputs(tx, state, view, true, scriptVerifyFlags & ~SCRIPT_VERIFY_CLEANSTACK, true, txdata, nFees)) {
        ...
        if (!CheckInputs(tx, state, view, true, MANDATORY_SCRIPT_VERIFY_FLAGS, true, txdata, nFees))
        {
...
bool CScriptCheck::operator()() {
    ...
    if (!VerifyScript(scriptSig, scriptPubKey, witness, nFlags, CachingTransactionSignatureChecker(ptxTo, nIn, amount, nFees, cacheStore, *txdata), &error)) {
        return false;
    }
    return true;
}
...
bool CheckInputs(const CTransaction& tx, CValidationState &state, const CCoinsViewCache &inputs, bool fScriptChecks, unsigned int flags, bool cacheStore, PrecomputedTransactionData& txdata, const CAmount& nFees, std::vector<CScriptCheck> *pvChecks)
{
    if (!tx.IsCoinBase())
    {
        ...
        if (fScriptChecks) {
            for (unsigned int i = 0; i < tx.vin.size(); i++) {
                ...
                // Verify signature
                CScriptCheck check(*coins, tx, i, flags, cacheStore, &txdata, nFees);
                if (pvChecks) {
                    ...
                    if (flags & STANDARD_NOT_MANDATORY_VERIFY_FLAGS) {
                        ...
                        CScriptCheck check2(*coins, tx, i,
                                flags & ~STANDARD_NOT_MANDATORY_VERIFY_FLAGS, cacheStore, &txdata, nFees);
                        if (check2())
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        if (!tx.IsCoinBase())
        {
            ...
            if (!CheckInputs(tx, state, view, fScriptChecks, flags, fCacheResults, txdata[i], nTxFees, nScriptCheckThreads ? &vChecks : NULL))
