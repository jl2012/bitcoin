...
static std::pair<int, int64_t> CalculateSequenceLocks(const CTransaction &tx, int flags, std::vector<int>* prevHeights, const CBlockIndex& block)
{
    ...
    for (size_t txinIndex = 0; txinIndex < tx.vin.size(); txinIndex++) {
        ...
        if (txin.IsCoinbase() || (txin.nSequence & CTxIn::SEQUENCE_LOCKTIME_DISABLE_FLAG)) {
...
bool CheckTransaction(const CTransaction& tx, CValidationState &state, const bool& hardfork, bool fCheckDuplicateInputs)
{
    ...
    if (!hardfork && ::GetSerializeSize(tx, SER_NETWORK, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > MAX_BLOCK_BASE_SIZE)
        return state.DoS(100, false, REJECT_INVALID, "bad-txns-oversize");
    if (hardfork && GetTransactionSizeCost(tx) > MAX_BLOCK_WEIGHT * HARDFORK_SCALE_FACTOR)
        return state.DoS(100, false, REJECT_INVALID, "bad-txns-oversize");
    ...
    if (hardfork) {
        ...
        if (tx.IsCoinBase(true)) {
            ...
            if (tx.vin[0].scriptWitness.stack.size()) {
                if (tx.vin[0].scriptWitness.stack.size() != 1 || tx.vin[0].scriptWitness.stack[0].size() > 252)
                    return state.DoS(100, false, REJECT_INVALID, "bad-coinbase-witness-size", true, strprintf("%s : invalid coinbase witness size", __func__));
            }
            start = 1;
        }
        for (unsigned int i = start; i < tx.vin.size(); i++) {
            if (tx.vin[i].IsCoinbase())
                return state.DoS(10, false, REJECT_INVALID, "bad-txns-prevout-coinbase");
        }
        return true;
    }
                              ...
                              std::vector<uint256>& vHashTxnToUncache)
{
    ...
    bool hardforkEnabled = (chainActive.Tip()->nHeight + 1 < HARDFORK_HEIGHT);
...

    {
        ...
        int64_t nTxWeightNew = GetNewTransactionWeight(tx, view, scriptVerifyFlags);
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    if (nHeight >= HARDFORK_HEIGHT) {
        int nMod = nHeight % consensusParams.nSubsidyHalvingInterval;
        unsigned int nPowTargetBlock = consensusParams.nPowTargetTimespan / consensusParams.nPowTargetSpacing;
        if (nMod >= consensusParams.nSubsidyHalvingInterval - nPowTargetBlock)
            nSubsidy -= (nSubsidy >> 2);
        else if (nMod < nPowTargetBlock && halvings > 0) {
            CAmount nLastSubsidy = (50 * COIN) >> (halvings - 1);
            nSubsidy += (nLastSubsidy >> 2);
        }
    }
    return nSubsidy;
}
...
void UpdateCoins(const CTransaction& tx, CCoinsViewCache& inputs, CTxUndo &txundo, int nHeight)
{
    ...
    // add outputs
    inputs.ModifyNewCoins(tx.GetHash(), tx.IsCoinBase(false))->FromTx(tx, nHeight);
}
...
bool CheckTxInputs(const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, int nSpendHeight)
{
        ...
        for (unsigned int i = start; i < tx.vin.size(); i++)
        {
            ...
            if (coins->IsCoinBase()) {
                if (!tx.IsCoinBase(true) && (nSpendHeight - coins->nHeight < COINBASE_MATURITY))
                    return state.Invalid(false,
...
uint32_t ComputeBlockVersion(const CBlockIndex* pindexPrev, const Consensus::Params& params)
{
    LOCK(cs_main);
...
public:
    ...
    {
        return ((pindex->nDeploymentSoft & VERSIONBITS_TOP_MASK) == VERSIONBITS_TOP_BITS) &&
               ((pindex->nDeploymentSoft >> bit) & 1) != 0 &&
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    std::vector<CAmount> vTxFees(block.vtx.size());
    std::vector<int64_t> vTxWeight(block.vtx.size());
    vTxFees[0] = 0;
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        if (!tx.IsCoinBase(false))
        {
            ...
            for (size_t j = start; j < tx.vin.size(); j++) {
        ...
        if (!tx.IsCoinBase(false))
        {
            vTxFees[i] = view.GetValueIn(tx);
            if (!tx.IsCoinBase(true))
                vTxFees[i] -= tx.GetValueOut();
            nFees += vTxFees[i];
...
void static UpdateTip(CBlockIndex *pindexNew, const CChainParams& chainParams) {
    ...
    if (!IsInitialBlockDownload())
    {
        ...
        {
            uint32_t nExpectedVersion = ComputeBlockVersion(pindex->pprev, chainParams.GetConsensus());
            if (pindex->nDeploymentSoft > VERSIONBITS_LAST_OLD_BLOCK_VERSION && (pindex->nDeploymentSoft & ~nExpectedVersion) != 0)
                ++nUpgraded;
            pindex = pindex->pprev;
        }
        if (nUpgraded > 0)
    ...
    LogPrintf("%s: new best=%s height=%d deploySoft=0x%08x log2_work=%.8g tx=%lu date='%s' progress=%f cache=%.1fMiB(%utx)", __func__,
      chainActive.Tip()->GetBlockHash().ToString(), chainActive.Height(), chainActive.Tip()->nDeploymentSoft,
...
bool static DisconnectTip(CValidationState& state, const CChainParams& chainparams, bool fBare = false)
{
    ...
    if (!fBare) {
        ...
        for (const auto& it : block.vtx) {
            ...
            CValidationState stateDummy;
            if (tx.IsCoinBase(true) || !AcceptToMemoryPool(mempool, stateDummy, it, false, NULL, true)) {
...
CBlockIndex* AddToBlockIndex(const CBlockHeader& block)
{
    ...
    pindexNew->nTimeMax = (pindexNew->pprev ? std::max(pindexNew->pprev->nTimeMax, pindexNew->nTTime) : pindexNew->nTTime);
    pindexNew->InitialiseFromPrev();
...
bool CheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // Size limits
    bool hardforkEnabled = (block.nHeight >= HARDFORK_HEIGHT);
    if (!hardforkEnabled) {
        if (block.vtx.empty() || block.vtx.size() > MAX_BLOCK_BASE_SIZE || ::GetSerializeSize(block, SER_NETWORK, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS | SERIALIZE_BLOCK_LEGACY) > MAX_BLOCK_BASE_SIZE)
        ...
        if (block.vtx.empty() || block.vtx.size() * SIZE_SCALE_FACTOR * MIN_TX_SIZE > MAX_BLOCK_WEIGHT * HARDFORK_SCALE_FACTOR)
            return state.DoS(100, false, REJECT_INVALID, "bad-blk-length", false, "size limits failed");
        if (block.nTxsCount != block.vtx.size())
            return state.DoS(100, false, REJECT_INVALID, "bad-blk-txs-count", false, "number of transactions mismatch");
        ...
        if (nSizeCost > MAX_BLOCK_WEIGHT * HARDFORK_SCALE_FACTOR)
            return state.DoS(100, false, REJECT_INVALID, "bad-blk-length", false, "size limits failed");
    }

    // First transaction must be coinbase, the rest must not be
    if (block.vtx.empty() || !block.vtx[0]->IsCoinBase(hardforkEnabled))
        return state.DoS(100, false, REJECT_INVALID, "bad-cb-missing", false, "first tx is not coinbase");
    for (unsigned int i = 1; i < block.vtx.size(); i++)
        if (block.vtx[i]->IsCoinBase(hardforkEnabled))
            return state.DoS(100, false, REJECT_INVALID, "bad-cb-multiple", false, "more than one coinbase");
    if (block.hashPrevBlock != block.vtx[0]->vin[0].prevout.hash && hardforkEnabled)
        return state.DoS(100, false, REJECT_INVALID, "bad-cb-prevout-hash", false, "the first prevout of first tx is not hashPrevBlock");
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const Consensus::Params& consensusParams, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    if((block.nDeploymentSoft < 2 && nHeight >= consensusParams.BIP34Height) ||
       (block.nDeploymentSoft < 3 && nHeight >= consensusParams.BIP66Height) ||
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, const CBlockIndex* pindexPrev)
{
    ...
    if (nHeight >= consensusParams.BIP34Height && nHeight < HARDFORK_HEIGHT)
    {
    ...
    // failed).
    if (nHeight < HARDFORK_HEIGHT && GetBlockWeight(block) > MAX_BLOCK_WEIGHT) {
...
bool static LoadBlockIndexDB(const CChainParams& chainparams)
{
    ...
    BOOST_FOREACH(const PAIRTYPE(int, CBlockIndex*)& item, vSortedByHeight)
    {
        ...
        pindex->nTimeMax = (pindex->pprev ? std::max(pindex->pprev->nTimeMax, pindex->nTTime) : pindex->nTTime);
