...
uint256 CBlockHeader::GetHashHB() const
{
    ...
    ser_writedata8(writer, nDeploymentHard >> 8);
...
{
    std::stringstream s;
    s << strprintf("CBlock(hash=%s, height=%u, deploySoft=0x%08x, deployHard=0x%04x, hashPrevBlock=%s, hashMerkleRoot=%s, hashMerkleRootWitness=%s, hashMerkleSumRoot=%s, nTime=%u, nBits=0x%08x, nNonce=%u:%u:%s, vtx=%u)\n",
        GetHash().ToString(),
        nHeight,
        nDeploymentSoft,
        nDeploymentHard,
...
int64_t GetBlockWeight(const CBlock& block)
{
    ...
    return ::GetSerializeSize(block, SER_NETWORK, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS | SERIALIZE_BLOCK_LEGACY) * (WITNESS_SCALE_FACTOR - 1) + ::GetSerializeSize(block, SER_NETWORK, PROTOCOL_VERSION | SERIALIZE_BLOCK_LEGACY);
}