...
bool static IsLowDERSignature(const valtype &vchSig, ScriptError* serror = NULL) {
        ...
        while (pc < pend)
        {
            ...
            switch (opcode)
            {
                ...
                case OP_CHECKSIGVERIFY:
                {
                    ...
                    if ((sigversion != SIGVERSION_WITNESS_V1 && !CheckSignatureEncoding(vchSig, flags, serror)) || !CheckPubKeyEncoding(vchPubKey, flags, sigversion, serror)) {
                        //serror is set
                        return false;
                    }
                    bool fSuccess = checker.CheckSig(vchSig, vchPubKey, scriptCode, sigversion, prevScript, hashScript, sigScriptCode, posSigScriptCode, fSigScriptCodeUncommitted);
                ...
                case OP_CHECKMULTISIGVERIFY:
                {
                    ...
                    while (fSuccess && nSigsCount > 0)
                    {
                        ...
                        // Check signature
                        bool fOk = checker.CheckSig(vchSig, vchPubKey, scriptCode, sigversion, prevScript, hashScript, sigScriptCode, posSigScriptCode, fSigScriptCodeUncommitted);
...
uint256 SignatureHash(const CPubKey& pubkey, const CScript& scriptCodeIn, const uint256& hashScriptIn, std::vector<CScript> sigScriptCode, const CTransaction& txTo, unsigned int nIn, const unsigned int& nOut, unsigned int nHashType, const CAmount& amountIn, const CAmount& nFeesIn, SigVersion sigversion, const PrecomputedTransactionData* cache)
{
    if (sigversion == SIGVERSION_WITNESS_V1) {
        ...
        CAmount amount = amountIn;
        CScript scriptPubKey = scriptCodeIn;
        COutPoint prevout = txTo.vin[nIn].prevout;

        if (nHashType & SIGHASHV2_THISINPUT) {
            ...
            prevout.SetNull();
            if (!(nHashType & SIGHASHV2_AMOUNT))
                amount = -1;
            if (!(nHashType & SIGHASHV2_PROGRAM))
                scriptPubKey.clear();
        }
...
bool ToDERSig(vector<unsigned char>& vchSig)
{
    if (vchSig.size() < 64)
        return false;
    valtype r, s;
...
bool TransactionSignatureChecker::CheckSig(const vector<unsigned char>& vchSigIn, const vector<unsigned char>& vchPubKey, const CScript& scriptCodeIn, SigVersion sigversion, const CScript& prevScript, const uint256& hashScript, const std::vector<CScript>& sigScriptCode, const size_t& posSigScriptCode, unsigned int fSigScriptCodeUncommitted) const
{
    ...
    CScript scriptCode = (sigversion <= SIGVERSION_WITNESS_V0 ? scriptCodeIn : prevScript);
    ...
    if (sigversion == SIGVERSION_WITNESS_V1) {
        int mask = 0;
        for (size_t i = 0; i < posSigScriptCode; i++)
            mask |= (1U << i);
        fSigScriptCodeUncommitted &= ~((nHashType >> 8) & 0x3f & mask);
    }
    return true;
}
...
size_t static WitnessSigOps(int witversion, const std::vector<unsigned char>& witprogram, const CScriptWitness& witness, int flags)
{
    if (witversion == 0) {
        ...
        std::vector<CScript> keyScriptCode, sigScriptCode;
        size_t nSigOp = 0;
        if (IsMASTStack(witness, nMASTVersion, path, position, stack, keyScriptCode) && nMASTVersion == 0 && ParseMASTV0Stack(stack, sigScriptCode)) {
            for (size_t i = 0; i < sigScriptCode.size(); i++)
                nSigOp += sigScriptCode[i].GetSigOpCount(true);
            for (size_t i = 0; i < keyScriptCode.size(); i++)
                nSigOp += keyScriptCode[i].GetSigOpCount(true);
        }
        return nSigOp;
    }
