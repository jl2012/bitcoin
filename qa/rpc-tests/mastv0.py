...
def uint32tohex(int):
    hex = struct.pack('<I', int)
    while (hex[-1] == 0):
        hex = hex[:-1]
    if (len(hex) and hex[-1] & 0x80):
        hex = hex + b'\x00'
    return hex

def int64tohex(int):
    hex = struct.pack('<q', int)
    while (len(hex) and hex[-1] == 0):
        hex = hex[:-1]
    if (len(hex) and hex[-1] & 0x80):
        hex = hex + b'\x00'
    return hex
...
class SegWitTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        amount = [630 * 1000 * 1000, 630 * 1000 * 1000, 640 * 1000 * 1000, 620 * 1000 * 1000, 610 * 1000 * 1000, 630 * 1000 * 1000, 600 * 1000 * 1000, 600 * 1000 * 1000]
        ...
        tx.vout.append(CTxOut(amount[6], CScript([OP_1, pubkey1])))
        tx.vout.append(CTxOut(amount[7], CScript([OP_1, pubkey2])))
        signresults = self.nodes[0].signrawtransaction(bytes_to_hex_str(tx.serialize_without_witness()))['hex']
        tx.deserialize(BytesIO(hex_str_to_bytes(signresults)))
        txid = self.tx_submit(0, tx)
        ...
        for i in range(10):
            self.fee -= pay
            self.tx.vout.append(CTxOut(pay, CScript([b'\x6a' + chr(i).encode('latin-1')])))
            pay -= 10 * 1000 * 1000
        for i in range(250):
            self.tx.vout.append(CTxOut(0, CScript([b'\x6a' + chr(i).encode('latin-1')])))
        ...
        self.tx.wit.vtxinwit[6].scriptWitness.stack = [signone1, b'']
        self.tx.wit.vtxinwit[7].scriptWitness.stack = [signone2, b'']
        ...
        self.rbf()
        sig1 = sign_mast(key1, mast1, 4, self.tx, 0, 0, 0x3000, amount[0], self.fee, sigcode4)
        sig2 = sign_mast(key2, mast1, 4, self.tx, 0, 0, 0x3000, amount[0], self.fee, sigcode4)
        ...
        self.rbf()
        sigcode5 = [CScript([OP_PUSHTXDATA, OP_EQUALVERIFY])]
        sig = sign_mast(key1, mast1, 0, self.tx, 2, 0, 0x3f00, amount[0], self.fee, sigcode5)
        self.tx.wit.vtxinwit[2].scriptWitness.stack = [sig, b'\x01', b''] + get_mast_stack(mast1, 0, sigcode5)
        self.tx_submit(0, self.tx, EQUALFAIL)
        self.tx.wit.vtxinwit[2].scriptWitness.stack = [sig, b'\x02', b''] + get_mast_stack(mast1, 0, sigcode5)
        self.tx_submit(0, self.tx)

        self.rbf()
        self.tx.wit.vtxinwit[2].scriptWitness.stack = [sig, int64tohex(len(self.tx.vin)), b'\x01'] + get_mast_stack(mast1, 0, sigcode5)
        self.tx_submit(0, self.tx)

        self.rbf()
        self.tx.wit.vtxinwit[2].scriptWitness.stack = [sig, int64tohex(len(self.tx.vout)), b'\x02'] + get_mast_stack(mast1, 0, sigcode5)
        self.tx_submit(0, self.tx)

        self.rbf()
        self.tx.wit.vtxinwit[2].scriptWitness.stack = [sig, int64tohex(amount[2]), b'\x03'] + get_mast_stack(mast1, 0, sigcode5)
        self.tx_submit(0, self.tx)

        self.rbf()
        self.tx.wit.vtxinwit[2].scriptWitness.stack = [sig, int64tohex(self.fee), b'\x04'] + get_mast_stack(mast1, 0, sigcode5)
        self.tx_submit(0, self.tx)

        self.rbf()
        self.tx.wit.vtxinwit[2].scriptWitness.stack = [sig, b'\xff\xff\xff\xff', b'\x05'] + get_mast_stack(mast1, 0, sigcode5)
        self.tx_submit(0, self.tx, EQUALFAIL)
        self.tx.nVersion = -1
        for i in range(len(self.tx.vin)):
            self.tx.vin[i].nSequence = 0
        self.tx_submit(0, self.tx, EQUALFAIL)
        self.tx.wit.vtxinwit[2].scriptWitness.stack = [sig, b'\xff\xff\xff\xff\x00', b'\x05'] + get_mast_stack(mast1, 0, sigcode5)
        self.tx_submit(0, self.tx)

        self.rbf()
        self.tx.nLockTime = 255
        self.tx.wit.vtxinwit[2].scriptWitness.stack = [sig, b'\xff\x00', b'\x06'] + get_mast_stack(mast1, 0, sigcode5)
        self.tx_submit(0, self.tx)

        self.rbf()
        self.tx.wit.vtxinwit[2].scriptWitness.stack = [sig, int64tohex(len(self.tx.serialize_without_witness())), b'\x07'] + get_mast_stack(mast1, 0, sigcode5)
        self.tx_submit(0, self.tx)

        self.rbf()
        self.tx.wit.vtxinwit[2].scriptWitness.stack = [sig, int64tohex(len(self.tx.serialize_with_witness())), b'\x08'] + get_mast_stack(mast1, 0, sigcode5)
        self.tx_submit(0, self.tx)

        self.rbf()
        self.tx.wit.vtxinwit[2].scriptWitness.stack = [sig, int64tohex(len(self.tx.serialize_without_witness()) * 3 + len(self.tx.serialize_with_witness())), b'\x09'] + get_mast_stack(mast1, 0, sigcode5)
        self.tx_submit(0, self.tx)

        self.rbf()
        self.tx.wit.vtxinwit[2].scriptWitness.stack = [sig, get_keyscripthash(mast1[0]), b'\x0a'] + get_mast_stack(mast1, 0, sigcode5)
        self.tx_submit(0, self.tx)

        self.rbf()
        self.tx.wit.vtxinwit[2].scriptWitness.stack = [sig, get_mast_spk(mast1), b'\x0b'] + get_mast_stack(mast1, 0, sigcode5)
        self.tx_submit(0, self.tx)

        self.rbf()
        self.tx.wit.vtxinwit[2].scriptWitness.stack = [sig, int64tohex(self.tx.vin[3].nSequence), b'\x03', b'\x0d'] + get_mast_stack(mast1, 0, sigcode5)
        self.tx_submit(0, self.tx)
        self.rbf()
        self.tx.wit.vtxinwit[2].scriptWitness.stack = [sig, int64tohex(self.tx.vin[2].nSequence), b'\x81', b'\x0d'] + get_mast_stack(mast1, 0, sigcode5)
        self.tx_submit(0, self.tx)

        self.rbf()
        sigcode6 = [CScript([OP_PUSHTXDATA, OP_ROT, OP_EQUALVERIFY, OP_EQUALVERIFY])]
        sig = sign_mast(key1, mast1, 0, self.tx, 2, 0, 0x3f00, amount[0], self.fee, sigcode6)
        self.tx.wit.vtxinwit[2].scriptWitness.stack = [sig, txhashrev, int64tohex(self.tx.vin[3].prevout.n), b'\x03', b'\x0c'] + get_mast_stack(mast1, 0, sigcode6)
        self.tx_submit(0, self.tx)
        self.rbf()
        self.tx.wit.vtxinwit[2].scriptWitness.stack = [sig, txhashrev, int64tohex(self.tx.vin[2].prevout.n), b'\x81', b'\x0c'] + get_mast_stack(mast1, 0, sigcode6)
        self.tx_submit(0, self.tx)

        self.rbf()
        self.tx.wit.vtxinwit[2].scriptWitness.stack = [sig, self.tx.vout[3].scriptPubKey, int64tohex(self.tx.vout[3].nValue), b'\x03', b'\x0f'] + get_mast_stack(mast1, 0, sigcode6)
        self.tx_submit(0, self.tx)
        self.rbf()
        self.tx.wit.vtxinwit[2].scriptWitness.stack = [sig, self.tx.vout[2].scriptPubKey, int64tohex(self.tx.vout[2].nValue), b'\x81', b'\x0f'] + get_mast_stack(mast1, 0, sigcode6)
        self.tx_submit(0, self.tx)

        self.rbf()
        sigcode6 = [CScript([OP_PUSHTXDATA, OP_2SWAP, OP_EQUALVERIFY, OP_ROT, OP_EQUALVERIFY, OP_EQUALVERIFY])]
        sig = sign_mast(key1, mast1, 0, self.tx, 2, 0, 0x3f00, amount[0], self.fee, sigcode6)
        self.tx.wit.vtxinwit[2].scriptWitness.stack = [sig, int64tohex(self.tx.vin[3].prevout.n), int64tohex(self.tx.vin[3].nSequence), txhashrev, b'\x03', b'\x0e'] + get_mast_stack(mast1, 0, sigcode6)
        self.tx_submit(0, self.tx)
        self.rbf()
        self.tx.wit.vtxinwit[2].scriptWitness.stack = [sig, int64tohex(self.tx.vin[2].prevout.n), int64tohex(self.tx.vin[2].nSequence), txhashrev, b'\x81', b'\x0e'] + get_mast_stack(mast1, 0, sigcode6)
        self.tx_submit(0, self.tx)


        # self.rbf()
        # self.tx.wit.vtxinwit[2].scriptWitness.stack = [sig, txhashrev, b'\x08', b'\x09'] + get_mast_stack(mast1, 0, sigcode5)
        # self.tx_submit(0, self.tx, INVALIDSTACK)
        # self.tx.wit.vtxinwit[2].scriptWitness.stack = [sig, txhashrev, b'\x08', b'\x0a'] + get_mast_stack(mast1, 0, sigcode5)
        # self.tx_submit(0, self.tx, INVALIDSTACK)
        # self.tx.wit.vtxinwit[2].scriptWitness.stack = [sig, txhashrev, b'\x08', b'\x0b'] + get_mast_stack(mast1, 0, sigcode5)
        # self.tx_submit(0, self.tx, INVALIDSTACK)
        # self.tx.wit.vtxinwit[2].scriptWitness.stack = [sig, txhashrev, b'\x08', b'\x0c'] + get_mast_stack(mast1, 0, sigcode5)
        # self.tx_submit(0, self.tx, INVALIDSTACK)
        # self.tx.wit.vtxinwit[2].scriptWitness.stack = [sig, txhashrev, b'\x03', b'\x09'] + get_mast_stack(mast1, 0, sigcode5)
        # self.tx_submit(0, self.tx)
        #
        # self.rbf()
        # self.tx.wit.vtxinwit[2].scriptWitness.stack = [sig, int64tohex(self.tx.vin[3].prevout.n), b'\x03', b'\x0a'] + get_mast_stack(mast1, 0, sigcode5)
        # self.tx_submit(0, self.tx)
        #
        # self.rbf()
        # self.tx.wit.vtxinwit[2].scriptWitness.stack = [sig, int64tohex(self.tx.vin[2].prevout.n), b'\x81', b'\x0a'] + get_mast_stack(mast1, 0, sigcode5)
        # self.tx_submit(0, self.tx)
        #
        # self.rbf()
        # self.tx.wit.vtxinwit[2].scriptWitness.stack = [sig, int64tohex(self.tx.vin[3].nSequence), b'\x03', b'\x0b'] + get_mast_stack(mast1, 0, sigcode5)
        # self.tx_submit(0, self.tx)
        #
        # self.rbf()
        # self.tx.wit.vtxinwit[2].scriptWitness.stack = [sig, self.tx.vout[3].scriptPubKey, b'\x03', b'\x0e'] + get_mast_stack(mast1, 0, sigcode5)
        # self.tx_submit(0, self.tx)
        #
        # self.rbf()
        # self.tx.wit.vtxinwit[2].scriptWitness.stack = [sig, self.tx.vout[2].scriptPubKey, b'\x81', b'\x0e'] + get_mast_stack(mast1, 0, sigcode5)
        # self.tx_submit(0, self.tx)
        #
        # self.rbf()
        # self.tx.wit.vtxinwit[2].scriptWitness.stack = [sig, int64tohex(self.tx.vout[3].nValue), b'\x03', b'\x0f'] + get_mast_stack(mast1, 0, sigcode5)
        # self.tx_submit(0, self.tx)

        self.rbf(10 * 1000 * 1000)
        sig2 = sign_mast(key2, mast1, 4, self.tx, 0, 0, 0xf0ff, amount[0], self.fee, sigcode4)
        self.tx.wit.vtxinwit[0].scriptWitness.stack = [b'\x01', sig2, sig3, sig4, signone1, signone1, signone1, signone1] + get_mast_stack(mast1, 4, sigcode4)
        self.tx_submit(0, self.tx)
        self.mine_and_clear_mempool(0)

    def rbf(self, fee = 2 * 1000 * 1000):
        self.fee += fee
