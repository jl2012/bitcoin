...
from test_framework.script import CScript, OP_DUP, OP_HASH160, OP_EQUALVERIFY, OP_CHECKSIG, OP_0, hash160, OP_EQUAL
...
class SegWitTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        for i in uncompressed_address:
            ...
            if (v['isscript']):
                ...
                # seen_before_add_script.append(p2wsh)    # Before fix
                unseen_script.append(p2wsh)  # After fix, bare P2WSH derived from a known P2SH with uncompressed keys should never be seen
                ...
                # seen_before_add_script.append(CScript(p2wpkh))  # Before fix
                unseen_script.append(p2wpkh)  # After fix, bare P2WPKH of a known uncompressed key should never be seen
    ...
    def mine_and_test_listunspent(self, script_list, seen):
        ...
        tx.rehash()
        aaa = self.nodes[0].signrawtransaction(bytes_to_hex_str(tx.serialize_without_witness()))['hex']
        txid = self.nodes[0].sendrawtransaction(aaa, True)
